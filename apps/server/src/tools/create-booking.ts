...
import { PropertyModel, BookingModel, ScheduleEventModel, nextBookingId } from '../shared/db.js';
...
import { withPropertyLock } from '../shared/booking-lock.js';

export async function executeCreateBooking(
  input: CreateBookingInput,
): Promise<CreateBookingResult> {
  const { property_id, guest_name } = input;
  const check_in = normalizeCheckIn(input.check_in);
  const check_out = normalizeCheckOut(input.check_out);
  ...
  // Lock on property to prevent double-booking race condition
  const { bookingId, totalEstimate } = await withPropertyLock(property_id, async () => {
    ...
    // Generate booking ID atomically via MongoDB counter
    ...
    return { bookingId: id, totalEstimate: nights * property.current_price };
  });
